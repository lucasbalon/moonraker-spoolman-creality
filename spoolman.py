...
#
# Compatibility fork for pre-refactor Moonraker builds — see README.md
# in this repo for details on why this exists and what changed.
...
class SpoolManager:
    def __init__(self, config: ConfigHelper) -> None:
        ...
        # Announcement feed registration is optional; skip quietly if this
        # Moonraker build's announcements component doesn't support it.
        announcements = self.server.lookup_component("announcements", None)
        if announcements is not None and hasattr(announcements, "register_feed"):
            try:
                announcements.register_feed("spoolman")
            except Exception:
                logging.exception("Failed to register spoolman announcement feed")
    ...
    def _register_endpoints(self) -> None:
        self.server.register_endpoint(
            "/server/spoolman/spool_id", ['GET', 'POST'],
            self._handle_spool_id_request,
        )
        self.server.register_endpoint(
            "/server/spoolman/proxy", ['POST'],
            self._proxy_spoolman_request,
        )
        self.server.register_endpoint(
            "/server/spoolman/status", ['GET'],
    ...
    async def _handle_klippy_ready(self) -> None:
        ...
        sub: Dict[str, Optional[List[str]]] = {"toolhead": ["position", "extruder"]}
        try:
            result = await self.klippy_apis.subscribe_objects(sub)
        except self.server.error:
            logging.exception("Error subscribing to toolhead")
            return
    ...
    async def _handle_status_update(self, data: Dict[str, Any]) -> None:
        toolhead: Optional[Dict[str, Any]] = data.get("toolhead")
        if toolhead is None:
            return
    ...
    async def report_extrusion(self, eventtime: float) -> float:
        ...
        for spool_id, used_length in pending_reports.items():
            ...
            if response.has_error():
                if response.status_code == 404:
                    # Since the spool is deleted we can remove any pending
                    # reports added while waiting for the request
                    ...
                    if not self._error_logged:
                        ...
                        logging.info(
                            f"Failed to update extrusion for spool id "
                            f"{spool_id}, received {error_msg}"
                        )
                    # Add missed reports back to pending reports for
                    # the next cycle
                    self._add_extrusion(spool_id, used_length)
                    continue
            self._error_logged = False
